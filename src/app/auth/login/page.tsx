...
function LoginForm() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (error) { toast.error(error.message) }
      ...
      router.push(lang ? `/dashboard?create=${encodeURIComponent(lang)}` : '/dashboard')
      router.refresh()
    }
    setLoading(false)
  }
    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg)', fontFamily: 'var(--font-stack)' }}>
      ...
      <nav className="flex items-center justify-between px-8 py-5 border-b border-white/10">
        ...
        <p className="text-white/30 text-xs">
          No account?{' '}
          <Link href="/auth/register" className="text-white hover:underline">Sign up free</Link>
        </p>
      </nav>
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-16">
        <div className="w-full max-w-md">
          {/* Header */}
          <div className="mb-10">
            <p className="text-white/30 text-xs uppercase tracking-[0.2em] mb-3">Welcome back</p>
            <h1 className="text-4xl font-bold tracking-tight mb-3">Sign in to CodeItUp</h1>
            ...
            {lang && (
              <div className="mt-4 inline-flex items-center gap-2 border border-white/10 px-3 py-1.5 text-xs text-white/60">
                ...
                A <span className="text-white font-semibold">{lang}</span> project will be created on sign-in
              </div>
            )}
          </div>
